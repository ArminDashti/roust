...
pub fn run(out_dir: &Path) -> Result<()> {
    ...
    let files: [(&str, &[String]); 4] = [
        ("ipv4_cidr.txt", &ipv4_cidr),
        ("ipv6_cidr.txt", &ipv6_cidr),
        ("ipv4.txt", &ipv4_plain),
        ("ipv6.txt", &ipv6_plain),
    ];
    for (name, _) in &files {
...
pub fn run_private_ips(out_dir: &Path) -> Result<()> {
    ...
    let cidrs: Vec<String> = serde_json::from_str(&body)
        .context("parse private_ips.json (expected JSON array of strings)")?;
    let plain: Vec<String> = cidrs.iter().map(|s| strip_subnet(s)).collect();
    let files: [(&str, &[String]); 2] = [
        ("private_ips_cidr.txt", &cidrs),
        ("private_ips.txt", &plain),
    ];
    for (name, _) in &files {
        let path = out_dir.join(name);
        if path.exists() {
...
mod tests {
    ...
    fn strip_subnet_ipv6() {
        assert_eq!(strip_subnet("2001:678:b0::/46"), "2001:678:b0::");
    }
    #[test]
    fn parse_sample_payload() {
        let j = r#"{"country":"Iran","countryCode":"IR","prefixes":{"ipv4":["1.2.3.0/24"],"ipv6":["2001:db8::/32"]}}"#;
