...
fn run_service() -> Result<()> {
    ...
    // Load config and open WinDivert before Running. On failure we must report
    // Stopped — otherwise SCM stays StartPending forever with a zombie process.
    let router = match (|| -> Result<_> {
        ...
        PacketRouter::with_interfaces(config, config_path)
