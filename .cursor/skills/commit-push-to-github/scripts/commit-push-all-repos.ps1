...
function Invoke-Git {
    ...
    try {
        ...
        return @{ Code = $code; Out = $text; TimedOut = $false }
    }
    catch {
...
function Get-GitHubToken {
    ...
    # Fallback: classic PAT also accepted as GITHUB_TOKEN
    ...
    if (-not $token) {
        Write-Color 'GITHUB_TOKEN_PAT / GITHUB_TOKEN is not set. Aborting.' 'Red'
...
function Enable-GitHubAuth {
    ...
    # Bypass Windows GCM stale creds; inject classic PAT for HTTPS github.com.
    # Note: PowerShell/git treat empty GIT_CONFIG_VALUE_* as missing — use !true no-op helper.
...
function Get-RemoteOwnerRepo {
    param([string]$RepoPath)
    # Prefer config value (not insteadOf-rewritten URL) so PAT never appears in parse path
    $r = Invoke-Git $RepoPath @('config', '--get', 'remote.origin.url')
    if ($r.Code -ne 0 -or -not $r.Out) {
        $r = Invoke-Git $RepoPath @('remote', 'get-url', 'origin')
    }
    if ($r.Code -ne 0 -or -not $r.Out) { return $null }
    # Allow dots in repo names (e.g. armindashti.github.io)
    if ($r.Out -match 'github\.com[:/]([^/]+)/([^/]+?)(?:\.git)?/?$') {
...
function Get-ScanRootsFromMarkdown {
    ...
    foreach ($line in (Read-RepoTextFile $Path) -split "`r?`n") {
        ...
        # Accept Windows drive paths with \ or /, and Unix absolute paths
        if ($text -match '^[A-Za-z]:[/\\]' -or $text -match '^/') {
            $expanded = [System.IO.Path]::GetFullPath(($text -replace '/', '\'))
...
function Find-LargeFilesInRepo {
    ...
    # Always return Object[] so .Count is safe under StrictMode
...
function Update-GitIgnoreAutoSection {
    ...
    $paths = @($ExtraPaths | Where-Object { $_ })
    ...
    foreach ($p in $paths) { [void]$existing.Add(($p -replace '\\', '/')) }
...
function Test-SensitiveFiles {
    param([string[]]$Files)
    foreach ($f in @($Files)) { if ($f -and ($f -match $Script:SecretPat)) { return $true } }
    return $false
}
...
function New-CommitMessage {
    ...
    $extList = @($files | ForEach-Object { if ($_ -match '\.([^.]+)$') { $Matches[1] } } | Sort-Object -Unique)
    $dirList = @($files | ForEach-Object { ($_ -split '[/\\]')[0] } | Sort-Object -Unique)
    $exts = ($extList | Select-Object -First 4) -join '/'
    $dirs = ($dirList | Select-Object -First 3) -join ', '
...
function Resolve-ConflictFile {
    ...
    while ($i -lt $lines.Count) {
        if ($lines[$i] -match '^<<<<<<< ') {
            ...
            foreach ($line in $chosen) { $resolved.Add([string]$line) }
        }
