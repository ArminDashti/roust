...
.DESCRIPTION
  1) Stops matching app process(es) and Windows service (so files are not locked)
  2) Builds the app for Windows
  3) Installs (copies) the .exe and required runtime files into an output folder (default: .\release)
  4) Starts the installed app from that folder
...
function Show-Help {
    Write-Host ""
    Write-Host "$AppName export-exe.ps1 - stop, build, install, run .exe" -ForegroundColor White
    Write-Host ""
    Write-Host "Usage:" -ForegroundColor Cyan
    Write-Host "  .\export-exe.ps1 [--out=<path>] [--skip-stop] [--skip-run] [--help]"
    Write-Host ""
    Write-Host "Parameters:" -ForegroundColor Cyan
    Write-Host "  --out=<path>      Folder to install built binaries into (default: .\release)"
    Write-Host "  --skip-stop       Do not stop running app/service before build"
    Write-Host "  --skip-run        Do not start the installed app after install"
    Write-Host "  --help            Show this help"
    Write-Host ""
    Write-Host "Examples:" -ForegroundColor Cyan
    Write-Host "  .\export-exe.ps1"
    Write-Host "  .\export-exe.ps1 --out=.\release"
    Write-Host "  .\export-exe.ps1 --skip-run"
    Write-Host ""
    Write-Host "Output:" -ForegroundColor Cyan
    Write-Host "  Install folder: .\release\$AppExe  (+ required runtime files)"
    Write-Host ""
}
...
try {
    if (-not $SkipStop) {
        Invoke-Step "stop all app processes/service" {
            Stop-AppOrService
        }
    } else {
    ...
    # AGENT: COPY_BLOCK — install (copy) exe + required runtime files into $OutDir
    # Tauri: also copy WebView2Loader.dll from the same release dir as the exe.
    Invoke-Step "install into release folder" {
        Copy-Item -Force $BuiltExe (Join-Path $OutDir $AppExe)
        Write-Ok ("Installed $AppExe -> " + $OutDir)
    }
    ...
    Write-Host ("  Install: " + $ReleasedExe) -ForegroundColor Green
    Write-Host ""

    if (-not $SkipRun) {
        Invoke-Step "run installed app" {
